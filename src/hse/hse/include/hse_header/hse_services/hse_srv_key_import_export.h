...
typedef struct
{
    ...
    /** @brief   INPUT: Specifies usage flags, restriction access, key length in bits, etc for the key (see hseKeyInfo_t). <br>
     *                  NOTE:
     ...
     *           A asymmetric private key should always be imported together with the public key. <br>
     *           - pKey[0]:
     ...
     *              - ED25519 point x <br>
     *           - pKey[1]:
     *              - RSA public exponent e (big-endian). <br>
     *           - pKey[2]:
    ...
    uint16_t        keyLen[3];
    uint8_t         reserved[2];
    /** @brief   INPUT: Cipher parameters are used only if the cipherKeyHandle is not HSE_INVALID_KEY_HANDLE. <br>
     *                  NOTE:
    ...
    */
    struct
    {
        ...
        /** @brief Symmetric, asymmetric  and AEAD cipher scheme. <br>
                   NOTE:
    ...
    struct
    {
        ...
        /** @brief   INPUT: Byte length(s) of the authentication tag(s). <br>
         *                   NOTE:
         *                   - For MAC and RSA signature,  only authLen[0] is used.
         *                   - Both lengths are used for (R,S) (ECC or ED25519). */
        uint16_t          authLen[2];
        /** @brief   INPUT: Address(es) to authentication tag. <br>
         *                   NOTE:
...
typedef struct
{
    ...
    HOST_ADDR           pKeyInfo;
    /** @brief   OUTPUT: Addresses where to fill to key values. <br>
     *           - pKey[0]:
    ...
    struct
    {
        ...
        hseAuthScheme_t   authScheme;
        /** @brief   OUTPUT: Address(es) for the length(s) (uin16_t values) of the authentication tag. <br>
         *                   NOTE:
         *                   - For MAC and RSA signature,  only pAuthLen[0] is used.
         *                   - Both lengths are used for (R,S) (ECC or ED25519). */
        HOST_ADDR         pAuthLen[2];
        /** @brief   OUTPUT: Address of authentication tag. <br>
         *                   NOTE:
         *                   - For MAC and RSA signature,  only pAuth[0] is used.
         *                   - Both pointers are used for (R,S) (ECC or ED25519). */
        HOST_ADDR         pAuth[2];
