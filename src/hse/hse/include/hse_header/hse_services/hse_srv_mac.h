...
typedef struct
{
     ...
     *           STREAMING USAGE: Used in all steps.<br>
    ...
    /** @brief   INPUT: The input message. <br>
     *           NOTE: The input message for GMAC is the AAD (as specified by AEAD-GCM). <br>
     *           STREAMING USAGE: Used in all steps, but ignored when inputLength is zero */
    HOST_ADDR       pInput;

    /** @brief   INPUT/OUTPUT: Holds the address to a memory location (an uint32_t variable) in which the tag length in bytes is stored. <br>
     *                         - GENERATE:
 ...
 *                 These can be used to verify both the integrity and authenticity of a message.
 *
 *                 This FAST CMAC version can provide improved performance for CAN frames.
 *
 */
typedef struct
{
    ...
    /** @brief   INPUT: Length of the input message. Max value is 255 bytes <br> */
    uint8_t       inputLength;

    /** @brief   INPUT: Specifies the direction: generate/verify. */
    hseAuthDir_t   authDir;

    /** @brief   INPUT/OUTPUT: Holds tag length in bytes. <br>
     *           - GENERATE:
     *             - On calling service (input), this parameter shall contain the size of the buffer provided by pTag.
     *             - Valid tag lengths are [4, cipher-block-length]. Tag-lengths greater than cipher-block-length will be
     *               truncated to cipher-block-length.
     *           - VERIFY:
     *             - On calling service (input), this parameter shall contain the tag-length to be verified.
