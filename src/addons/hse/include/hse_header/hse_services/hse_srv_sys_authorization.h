...
*    |IN_FIELD    |        U        |       U       |     U     |     U     |
*
*   After reset, the SYS rights are synchronized with Life cycle (LC) and CUST/OEM START_AS_USER policy attributes (see CUST/OEM policy attributes).  <br>
*      - if LC = CUST_DEL:
...
*      In this case, the authorization cannot be performed if any SHE key has the WRITE_PROTECTED flag set. <br>
...
typedef struct
{
    ...
    /** @brief   INPUT: Authentication scheme. ONLY RSA, ECDSA signature and CMAC are supported. <br>
